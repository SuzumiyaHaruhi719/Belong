...
## PART D: NEW PLATFORM SCREENS

The following screens extend the app from a 4-tab layout to a 5-tab platform. They cover the Posts tab, expanded Chat & Notifications tab, enhanced Profile flows, and new bottom sheets. All screens follow the global design principles, spacing rules, and component standards defined in Part A.
...
**Purpose:** Primary discovery feed for user-generated posts. Pinterest-style masonry layout optimized for visual content browsing.

**Navigation bar**
- "Posts" left-aligned title (SF Pro Display, 28pt, bold, textPrimary)
- Right: notification bell icon (24pt) with unread badge (red circle, 8pt)

**Search bar**
- Full-width, 40pt height, 12pt corner radius, surfaceSecondary background
- Placeholder: "Search posts, tags, people..." with magnifying glass icon leading
- Tap → keyboard + recent searches dropdown

**Filter chips (below search bar)**
- Horizontal scroll row, 8pt gap between chips
- Chips: "For You" (default selected, terracotta background), "Following", "Nearby", trending tags (dynamic)
- Selected state: terracotta background, white text
- Unselected state: surfaceSecondary background, textSecondary text
- 16pt vertical padding above/below chip row

**Waterfall grid**
- 2-column masonry layout, 8pt gap between columns, 8pt gap between rows
- 20pt left/right screen margins
- Each card:
  - Image thumbnail: variable height based on original aspect ratio, 12pt corner radius, fills column width
  - Author overlay at bottom of image: semi-transparent black gradient (bottom 40pt)
  - Avatar (24pt, circular) + display name (SF Pro Text, 13pt, white, medium) left-aligned in overlay
  - Heart icon (12pt) + like count (SF Pro Text, 12pt, white) right-aligned in overlay
- Tap card → S21 Post Detail (modal presentation)
- Long-press card → haptic feedback + quick actions (save, share, hide)

**Pull-to-refresh**
- Terracotta-colored spinner (same as Home tab)
- 48pt pull distance to trigger

**Infinite scroll**
- Skeleton cards appear at bottom: 6 cards in 2-column grid, pulsing surfaceSecondary
- Skeleton cards match approximate aspect ratios (alternate between short and tall)

**Empty state**
- Centered vertically in content area
- Illustration: stylized camera/photo icon (64pt, terracotta, 0.5 opacity)
- Headline: "No posts yet" (SF Pro Display, 20pt, bold, textPrimary)
- Body: "Follow people or explore tags to fill your feed" (SF Pro Text, 15pt, textSecondary, center-aligned)

**Loading state**
- 6 skeleton cards in 2-column grid
- Cards pulse between surfaceSecondary and surfaceSecondary at 0.6 opacity
- 1.2s animation cycle

**Accessibility**
- Each card announces: "Post by [name], [like count] likes" via VoiceOver
- Grid navigable via VoiceOver rotor (headings mode skips to filter chips)
- Search bar labeled "Search posts, tags, and people"

---

#### S21 — Post Detail

**Purpose:** Full immersive view of a single post with comments and social actions.

**Presentation:** Full-screen modal, presented over feed with drag-to-dismiss from top edge.

**Image carousel**
- Horizontal swipeable, full-width
- Variable height: max 70% of screen height, min 200pt
- Page dots centered below images (8pt dots, 6pt gap, active = terracotta, inactive = surfaceSecondary)
- Pinch-to-zoom enabled (up to 3x)
- Double-tap to toggle between fit and fill
- Single image: no page dots, still supports zoom

**Author row (below images, 16pt top padding)**
- Avatar (36pt, circular) + display name (SF Pro Text, 16pt, semibold, textPrimary) + school name (SF Pro Text, 13pt, textSecondary)
- Right-aligned: Follow button (compact, 28pt height)
  - "Follow" state: terracotta background, white text
  - "Following" state: surfaceSecondary background, textSecondary text
  - If own post: no follow button, replaced with (...) menu

**Content text (16pt top padding)**
- SF Pro Text, 15pt, textPrimary, leading 22pt
- #hashtag highlighting: terracotta color, tappable → filtered feed for that tag
- @mention highlighting: terracotta color, tappable → S51 Other User Profile
- Expandable: if > 6 lines, truncate with "...more" (textSecondary, tappable)

**Tag chips (8pt below content)**
- Horizontal scroll row of tag chips (same style as filter chips, all unselected state)
- Tappable → filtered feed for that tag

**Action bar (16pt top padding, full-width, 48pt height)**
- Left group:
  - Heart: SF Symbol "heart" / "heart.fill" (24pt), toggle with scale animation
  - Like count (SF Pro Text, 14pt, textSecondary) right of heart
  - Comment: SF Symbol "bubble.left" (24pt) + count
- Right group:
  - Bookmark: SF Symbol "bookmark" / "bookmark.fill" (24pt), toggle
  - Share: SF Symbol "square.and.arrow.up" (24pt) → BS05 Share Post Sheet
- All icons: textSecondary when inactive, terracotta when active (heart, bookmark)

**"View Gathering" button (conditional)**
- Shown only if linked_gathering_id exists
- Full-width, 44pt height, surfaceSecondary background, 12pt radius
- SF Symbol "calendar" + "View Gathering" (SF Pro Text, 15pt, semibold, terracotta)
- Tap → S13 Gathering Detail

**Comments section (24pt top padding)**
- Section header: "Comments" (SF Pro Text, 16pt, semibold) + count
- Scrollable within the modal, newest first
- Each comment:
  - Avatar (28pt, circular) + display name (SF Pro Text, 14pt, semibold) + timestamp (SF Pro Text, 12pt, textSecondary)
  - Comment text (SF Pro Text, 14pt, textPrimary, leading 20pt)
  - Reply button: "Reply" (SF Pro Text, 13pt, textSecondary)
  - Like button: small heart (16pt) + count
- Nested replies:
  - Indented 24pt from parent
  - Thin connecting line (1pt, borderDefault) from parent avatar to reply
  - Max nesting depth: 3 levels, then "View more replies" link

**Fixed bottom composer**
- Pinned to bottom, above safe area
- Current user avatar (28pt) + text field (surfaceSecondary, 36pt height, 18pt radius) + "Post" button
- "Post" button: terracotta, disabled (0.4 opacity) until text entered
- Text field placeholder: "Add a comment..."
- Keyboard avoidance: composer moves up with keyboard

**Empty comments state**
- Centered in comments area
- "No comments yet — be the first to share your thoughts" (SF Pro Text, 14pt, textSecondary, center-aligned)

**Loading state**
- Skeleton for image area (full-width rectangle, pulsing)
- Skeleton for author row + text (3 lines)
- Skeleton for 2 comment blocks

---

#### S22 — Create Post

**Purpose:** Compose and publish a new post with images, text, tags, and optional gathering link.

**Navigation bar**
- Left: "X" close button (SF Symbol "xmark", 20pt)
- Center: "New Post" (SF Pro Display, 17pt, semibold)
- Right: "Post" button (terracotta background, white text, 32pt height, 12pt radius)
  - Disabled state: gray background, 0.4 opacity (when no image added)
  - Loading state: spinner replacing text
  - Success state: checkmark icon, then auto-dismiss

**Image picker section**
- Grid of camera roll images (3-column, square thumbnails)
- First cell: camera icon for live capture
- Max 9 images selectable
- Selected images show numbered badge (terracotta circle, white number, top-right corner)
- Tap selected image again to deselect

**Selected images preview (shown when >= 1 image selected)**
- Horizontal scroll strip, 80pt height, 8pt gap
- Each thumbnail: 72pt square, 8pt radius
- "X" badge (top-right, 20pt, red circle with white X) to remove
- Drag to reorder (haptic feedback on grab)

**Text editor**
- Multi-line text input, min 100pt height, expands with content
- Placeholder: "Share your experience..." (SF Pro Text, 15pt, textSecondary)
- SF Pro Text, 15pt, textPrimary
- No character limit enforced
- Character count: shown bottom-right of text area (SF Pro Text, 12pt, textMuted), purely informational

**Hashtag autocomplete**
- Triggered when typing "#" followed by characters
- Dropdown below cursor position, max 5 suggestions
- Each suggestion: "#tagname" + post count (SF Pro Text, 14pt)
- Sources: trending tags + user's previously used tags
- Tap suggestion to insert

**@mention autocomplete**
- Triggered when typing "@" followed by characters
- Dropdown below cursor position, max 5 suggestions
- Each suggestion: avatar (24pt) + display name + username
- Tap suggestion to insert @mention

**Visibility picker**
- Segmented control, full-width minus margins, 36pt height
- Options: "Public" | "School Only" | "Followers Only"
- Default: "Public"
- Selected segment: terracotta background, white text
- Unselected: surfaceSecondary background, textSecondary text

**Link gathering (optional)**
- Row: SF Symbol "link" + "Link a gathering" (SF Pro Text, 15pt, textSecondary)
- Tap → dropdown/sheet listing user's upcoming and past gatherings (last 30 days)
- Selected gathering shown as chip below the row (removable)

**Post button states**
- Disabled: no image added → gray, non-tappable
- Enabled: at least 1 image → terracotta
- Loading: spinner inside button, non-tappable
- Success: checkmark animation → auto-navigate back to feed

**Discard confirmation**
- Triggered when X tapped and any content exists (images or text)
- Alert dialog:
  - Title: "Discard post?"
  - Body: "Your draft will be lost."
  - Actions: "Discard" (destructive, red) | "Keep Editing" (default)

---

### Chat & Notifications Tab Screens

---

#### S40 — Chat Tab (Split Header)

**Purpose:** Unified entry point for all notifications and messaging. Single tab with segmented navigation.

**Navigation bar**
- Title: "Activity" (SF Pro Display, 28pt, bold, textPrimary, left-aligned)
- Right: compose button (SF Symbol "square.and.pencil", 22pt) → S45 New DM

**Segmented control (below nav bar, 16pt padding)**
- Full-width minus margins, 36pt height, 12pt radius
- Two segments: "Notifications" | "Messages"
- Selected: terracotta background, white text
- Unselected: surfaceSecondary background, textSecondary text
- Notification badge on "Notifications" segment: red dot (8pt) if unread notifications exist
- Unread badge on "Messages" segment: count badge if unread messages exist

**Content area**
- Swipeable between two pages (matches segmented control)
- "Notifications" selected → S41 content
- "Messages" selected → S42 content

---

#### S41 — Notifications Feed

**Purpose:** Chronological feed of all social activity relevant to the user.

**Grouping**
- "Today" / "This Week" / "Earlier" section headers
- Section header: SF Pro Text, 14pt, semibold, textSecondary, uppercase, 8pt bottom padding

**Each notification row (full-width, 64pt min height, 12pt vertical padding)**
- Left: actor avatar (36pt, circular)
- Center: description text (SF Pro Text, 14pt, textPrimary) + timestamp (SF Pro Text, 12pt, textSecondary, below)
- Right (conditional): thumbnail image (44pt square, 8pt radius) for post/gathering notifications

**Notification types:**
| Type | Icon | Copy Pattern | Right Element |
|------|------|-------------|---------------|
| Like | heart.fill (red) | "[Name] liked your post" | Post thumbnail |
| Comment | bubble.left.fill (terracotta) | "[Name] commented: [preview...]" | Post thumbnail |
| Follow | person.fill.badge.plus (terracotta) | "[Name] started following you" | "Follow Back" button |
| Reminder | bell.fill (amber) | "[Gathering] starts in 1 hour" | Gathering thumbnail |
| New post | square.and.pencil (terracotta) | "[Name] shared a new post" | Post thumbnail |

**Unread indicator**
- Unread rows: surfaceSecondary background tint
- Read rows: white/clear background

**Nav bar action**
- "Mark all as read" text button (SF Pro Text, 14pt, terracotta) in top-right area
- Tap → all rows lose unread tint, badge clears

**Pull-to-refresh**
- Terracotta spinner

**Tap actions**
- Like/Comment/New post → S21 Post Detail
- Follow → S51 Other User Profile
- Reminder → S13 Gathering Detail

**Empty state**
- Centered illustration (bell icon, 64pt, terracotta, 0.5 opacity)
- "No notifications yet" (SF Pro Display, 20pt, bold)
- "Join gatherings and follow people to see activity here" (SF Pro Text, 15pt, textSecondary)

---

#### S42 — Conversation List (Messages)

**Purpose:** All direct messages and group chats in a single sorted list.

**Sort order:** Last message timestamp, newest first.

**Each conversation row (full-width, 72pt height, 12pt vertical padding)**
- Left: avatar (44pt, circular)
  - DMs: other user's avatar
  - Group chats: gathering emoji on colored circle background (or gathering image)
- Center:
  - Top line: name (SF Pro Text, 15pt, semibold, textPrimary) + timestamp (SF Pro Text, 12pt, textSecondary, right-aligned)
  - Bottom line: last message preview (SF Pro Text, 14pt, textSecondary, 1-line truncated)
  - Group chats: "[Sender]: [message]" format for preview
- Right: unread badge (terracotta circle, white count, 20pt diameter) if unread messages

**Swipe actions**
- Swipe left:
  - "Mute" (gray, bell.slash icon)
  - "Delete" (red, trash icon) → confirmation alert
- Muted conversations: muted icon (12pt) next to name

**New message FAB**
- Bottom-right, 56pt diameter, terracotta background, white "pencil.circle.fill" icon
- Shadow: 0pt x 4pt, 12pt blur, black 0.15 opacity
- Tap → S45 New DM

**Empty state**
- Centered illustration (chat bubble icon, 64pt, terracotta, 0.5 opacity)
- "No messages yet" (SF Pro Display, 20pt, bold)
- "Start a conversation with someone you've connected with" (SF Pro Text, 15pt, textSecondary)

---

#### S43 — DM Thread

**Purpose:** One-on-one private messaging between two users.

**Navigation bar**
- Left: back arrow (SF Symbol "chevron.left", 20pt)
- Center: other user's avatar (28pt) + display name (SF Pro Text, 16pt, semibold)
- Tap center → S51 Other User Profile

**Message bubbles**
- Current user (right-aligned):
  - Terracotta background, white text
  - SF Pro Text, 15pt, leading 20pt
  - Rounded corners: 16pt all, 4pt bottom-right
  - Max width: 75% of screen
- Other user (left-aligned):
  - White background, textPrimary text
  - SF Pro Text, 15pt, leading 20pt
  - Rounded corners: 16pt all, 4pt bottom-left
  - Small avatar (24pt) to the left of first message in a group
- Shared post message:
  - Embedded card: image thumbnail (120pt wide, aspect fit) + post title/preview + author name
  - Tap → S21 Post Detail
  - Card has surfaceSecondary background, 12pt radius, 1pt borderDefault border
- System messages:
  - Centered, SF Pro Text, 13pt, textMuted
  - Examples: "You are now mutuals — chat freely!"
- Timestamps: shown between message groups separated by > 5 minutes
  - Centered, SF Pro Text, 12pt, textMuted

**Mutual-follow gate**
- If not mutually following:
  - Banner at top of chat: surfaceSecondary background, 44pt height
  - "Follow each other to chat freely. You can send 1 message." (SF Pro Text, 13pt, textSecondary)
  - After 1 message sent by current user: composer disabled
  - Disabled composer text: "Waiting for [name] to follow you back" (SF Pro Text, 14pt, textMuted)
- If mutual: no banner, full chat access

**Typing indicator**
- Three animated dots in a bubble (left-aligned, same position as incoming message)
- Appears when other user is typing, disappears after 5s timeout

**Read receipts**
- Small "Seen" text (SF Pro Text, 11pt, textMuted) below last message read by other user
- Only shown for current user's messages

**Composer (fixed bottom)**
- Avatar (24pt, current user) + text field (surfaceSecondary, 36pt height, 18pt radius)
- Left of text field: image attach button (SF Symbol "photo", 20pt)
- Right of text field: share post button (SF Symbol "square.and.arrow.up", 20pt) → BS05
- Send button appears when text entered (SF Symbol "arrow.up.circle.fill", 28pt, terracotta)
- Keyboard avoidance: composer moves up

---

#### S44 — Group Chat (Enhanced)

**Purpose:** Extends existing S16 group chat with additional message types for the expanded platform.

**Additional message types (beyond existing S16 functionality):**

**Shared post messages**
- Same embedded card style as S43
- Shows: post image thumbnail + title + original author
- Tap → S21 Post Detail

**Image messages**
- Inline rendered, max width 240pt, aspect-fit
- Tap → full-screen image viewer with pinch-to-zoom
- 12pt corner radius

**System messages (expanded)**
- "[Name] joined the gathering" — when new attendee RSVPs
- "[Name] left the gathering" — when attendee cancels
- "Reminder: [Gathering] starts in 1 hour" — automated
- Centered, SF Pro Text, 13pt, textMuted, 8pt vertical padding

---

#### S45 — New DM

**Purpose:** Start a new direct message conversation by searching for users.

**Navigation bar**
- Left: "Cancel" text button (SF Pro Text, 16pt, terracotta)
- Center: "New Message" (SF Pro Display, 17pt, semibold)

**Search bar (16pt below nav)**
- Full-width minus margins, 40pt height, 12pt radius, surfaceSecondary background
- Placeholder: "Search by name or username..." with magnifying glass
- Real-time search as user types (300ms debounce)

**Recent conversations section (shown when search is empty)**
- Section header: "Recent" (SF Pro Text, 14pt, semibold, textSecondary, uppercase)
- Horizontal scroll of recent DM contacts: avatar (48pt) + name below (SF Pro Text, 12pt, center-aligned)
- Max 10 recent contacts shown

**Search results**
- Vertical list of user rows
- Each row: avatar (44pt) + display name (SF Pro Text, 15pt, semibold) + username (SF Pro Text, 14pt, textSecondary) + mutual badge if applicable
- Mutual badge: "Mutuals" chip (SF Pro Text, 11pt, terracotta text, terracotta/10% background, 4pt radius)
- Tap row → create or open existing conversation → navigate to S43

**Empty search results**
- "No users found" (SF Pro Text, 15pt, textSecondary, center-aligned)

---

### Profile Tab Screens (Expanded)

---

#### S50 — Profile (Enhanced)

**Purpose:** Expanded user profile with social stats and posts grid.

**Header section (same as existing profile)**
- Avatar (80pt, circular, centered) with edit badge for own profile
- Display name (SF Pro Display, 22pt, bold, textPrimary, centered)
- School name (SF Pro Text, 15pt, textSecondary, centered)
- City (SF Pro Text, 14pt, textSecondary, centered)

**Stats row (NEW — 16pt below header)**
- Three equal columns, centered text in each
- Each stat:
  - Count (SF Pro Display, 18pt, bold, textPrimary)
  - Label below (SF Pro Text, 12pt, textSecondary)
- Stats: "Following" | "Followers" | "Mutuals"
- Each tappable → respective list screen (S52, S53, S54)
- Dividers: 1pt vertical lines (borderDefault) between stats

**Segmented control (16pt below stats)**
- Full-width minus margins, 36pt height
- Four segments: "Overview" | "Activity" | "Hosted" | "Posts"
- Selected: terracotta underline (2pt), textPrimary text
- Unselected: textSecondary text

**Overview tab content**
- Cultural tags, saved gatherings, connections (same as existing profile content)

**Activity tab content**
- Browsing history + join history (same as current enhanced version)

**Hosted tab content**
- Host history with stats (same as current enhanced version)

**Posts tab content (NEW)**
- 3-column grid, 2pt gap between cells
- Each cell: square thumbnail (cropped from first image of post)
- Overlay on each: semi-transparent gradient at bottom, heart icon + like count (SF Pro Text, 11pt, white)
- Tap → S21 Post Detail
- Infinite scroll with skeleton loading (9 skeleton squares)

**Empty posts state**
- Centered in posts tab area
- "No posts yet — share your first experience!" (SF Pro Text, 15pt, textSecondary)
- "Create Post" button (terracotta, 44pt height, 12pt radius) → S22

---

#### S51 — Other User Profile

**Purpose:** View another user's public profile with follow and messaging actions.

**Layout:** Same as S50 but with action buttons instead of edit controls.

**Action buttons (below name/school, 16pt padding)**
- Primary: Follow/Unfollow button
  - "Follow" state: terracotta background, white text, 36pt height, 12pt radius
  - "Following" state: surfaceSecondary background, textSecondary text, same dimensions
  - "Follow Back" state: terracotta background, white text, label "Follow Back"
- Secondary: Message button (surfaceSecondary background, textPrimary text, SF Symbol "bubble.left" + "Message")
  - Shown if mutual follow or DM permissions allow
  - Tap → S43 DM Thread (create or open existing)
- (...) menu (top-right nav bar):
  - "Block [Name]" → BS06
  - "Report [Name]" → BS06

**Stats row:** Same as S50, shows their counts

**Segmented control:** "Overview" | "Posts" (simplified — no Activity/Hosted for other users unless they are public)

**Posts grid:** Same 3-column layout as S50, shows their public posts

**Blocked/private state**
- If user has blocked you or account is private:
  - Avatar + name shown
  - Content area: lock icon (48pt, textMuted) + "This account is private" (SF Pro Text, 16pt, textSecondary)
  - No posts grid, no stats

---

#### S52 — Following List

**Purpose:** View and manage the list of users the profile owner follows.

**Navigation bar**
- Left: back arrow (SF Symbol "chevron.left", 20pt)
- Center: "Following" (SF Pro Display, 17pt, semibold)

**Search bar (16pt below nav)**
- Full-width minus margins, 40pt height, 12pt radius, surfaceSecondary background
- Placeholder: "Search..." with magnifying glass
- Filters list in real-time

**User list**
- Each row (72pt height, 12pt vertical padding):
  - Avatar (44pt, circular)
  - Display name (SF Pro Text, 15pt, semibold, textPrimary)
  - Username below (SF Pro Text, 13pt, textSecondary)
  - Right: "Following" button (surfaceSecondary background, textSecondary text, 32pt height)
    - Tap → unfollow confirmation: "Unfollow [name]?" with "Unfollow" (destructive) and "Cancel"

**Empty state**
- "Not following anyone yet" (SF Pro Text, 15pt, textSecondary, centered)

---

#### S53 — Followers List

**Purpose:** View the list of users who follow the profile owner.

**Navigation bar**
- Left: back arrow
- Center: "Followers" (SF Pro Display, 17pt, semibold)

**Search bar:** Same as S52

**User list**
- Same layout as S52 but button states vary:
  - If you follow them back: "Following" button (surfaceSecondary)
  - If they follow you but you don't follow back: "Follow Back" button (terracotta background, white text)
  - If you don't follow them and they don't follow you (edge case): "Follow" button (terracotta)

**Empty state**
- "No followers yet" (SF Pro Text, 15pt, textSecondary, centered)

---

#### S54 — Mutuals List

**Purpose:** View users with mutual follow relationship. All mutuals have DM access.

**Navigation bar**
- Left: back arrow
- Center: "Mutuals" (SF Pro Display, 17pt, semibold)

**Search bar:** Same as S52

**User list**
- Same avatar + name layout as S52
- Right: "Message" button (surfaceSecondary background, terracotta text, SF Symbol "bubble.left", 32pt height)
  - Tap → S43 DM Thread

**Empty state**
- "No mutuals yet — follow people back to connect" (SF Pro Text, 15pt, textSecondary, centered)

---

#### S55 — Browse History (Expanded)

**Purpose:** View and manage browsing history across posts and gatherings.

**Navigation bar**
- Left: back arrow
- Center: "History" (SF Pro Display, 17pt, semibold)

**Segmented control (16pt below nav)**
- Two segments: "Posts" | "Gatherings"
- Selected: terracotta underline, textPrimary text
- Unselected: textSecondary text

**Posts history tab**
- Vertical list of viewed posts
- Each row (80pt height):
  - Thumbnail (60pt square, 8pt radius) from post's first image
  - Title/preview text (SF Pro Text, 14pt, textPrimary, 2-line truncated)
  - "Viewed [relative time]" (SF Pro Text, 12pt, textSecondary)
- Tap → S21 Post Detail
- "Clear all" button (top-right, SF Pro Text, 14pt, terracotta) → confirmation: "Clear post history?"

**Gatherings history tab**
- Same as current gatherings browsing history
- "Clear all" button with same confirmation pattern

**Empty states**
- Posts: "No posts viewed yet" (SF Pro Text, 15pt, textSecondary, centered)
- Gatherings: "No gatherings viewed yet" (SF Pro Text, 15pt, textSecondary, centered)
...
#### BS04 — Create Selector

**Purpose:** Disambiguation sheet when user taps the Create (+) tab in the 5-tab bar.

**Presentation:** Bottom sheet, drag handle, 50% screen height max.

**Drag handle**
- Centered, 36pt wide, 5pt tall, surfaceSecondary, 2.5pt radius

**Option cards (16pt padding, 12pt gap between cards)**
- Two large cards, full-width minus margins, 80pt height each, 12pt radius, surfaceSecondary background

- Card 1: "Host a Gathering"
  - Left: SF Symbol "calendar.badge.plus" (32pt, terracotta)
  - Right: title "Host a Gathering" (SF Pro Text, 17pt, semibold, textPrimary) + description "Plan an event for your community" (SF Pro Text, 13pt, textSecondary)
  - Tap → dismiss sheet → navigate to Host flow (S18)

- Card 2: "Create a Post"
  - Left: SF Symbol "camera.fill" (32pt, terracotta)
  - Right: title "Create a Post" (SF Pro Text, 17pt, semibold, textPrimary) + description "Share photos and experiences" (SF Pro Text, 13pt, textSecondary)
  - Tap → dismiss sheet → navigate to S22 Create Post

**Tap outside or drag down → dismiss**

---

#### BS05 — Share Post Sheet

**Purpose:** Share a post to DM conversations.

**Presentation:** Bottom sheet, drag handle, 60% screen height max.

**Drag handle:** Same as BS04.

**Header**
- "Share to..." (SF Pro Display, 17pt, semibold, textPrimary, left-aligned, 20pt padding)

**Recent DMs section**
- Horizontal scroll row, 16pt padding
- Each item: avatar (48pt, circular) + name below (SF Pro Text, 12pt, center-aligned, 1-line truncated)
- Max 10 recent contacts
- Tap → send shared_post message to that conversation → dismiss with toast

**All conversations section (below recent)**
- Section header: "All Conversations" (SF Pro Text, 14pt, semibold, textSecondary, uppercase)
- Search bar: 36pt height, "Search conversations..."
- Vertical list of conversation rows (same style as S42 but compact, 56pt height)
- Tap row → send shared_post message → dismiss with toast

**Toast on send**
- "Sent!" toast (bottom of screen, 44pt height, terracotta background, white text, 12pt radius)
- Auto-dismisses after 2 seconds

---

#### BS06 — Report / Block Sheet

**Purpose:** Report or block another user.

**Presentation:** Bottom sheet, drag handle, auto-height based on content.

**Drag handle:** Same as BS04.

**Options list (vertical, 16pt padding)**
- "Report [Name]" row:
  - SF Symbol "exclamationmark.triangle" (24pt, amber) + "Report" (SF Pro Text, 16pt, textPrimary)
  - Tap → expands to show reason picker inline (no new sheet)
  - Reason options (radio buttons):
    - "Spam"
    - "Harassment"
    - "Inappropriate content"
    - "Other" (with optional text field, 2-line, placeholder "Tell us more...")
  - "Submit Report" button (terracotta, 44pt height) after reason selected
  - Success: "Report submitted. Thank you." toast → dismiss

- "Block [Name]" row:
  - SF Symbol "hand.raised.fill" (24pt, red) + "Block" (SF Pro Text, 16pt, textPrimary)
  - Tap → inline confirmation:
    - "Block [Name]? They won't be able to see your profile or message you." (SF Pro Text, 14pt, textSecondary)
    - "Block" button (red background, white text, 44pt height) + "Cancel" (text button)
  - Success: "Blocked." toast → dismiss → return to previous screen

- "Cancel" row:
  - SF Pro Text, 16pt, textSecondary, center-aligned
  - Tap → dismiss sheet

---

### Edge Cases & Error Handling (New Screens)

| Scenario | Screen(s) | Behavior |
|----------|-----------|----------|
| Post has no images (shouldn't happen) | S21 | Show placeholder image with "Image unavailable" |
| Post deleted while viewing detail | S21 | Toast "This post has been deleted" → dismiss to feed |
| User blocked you while in their profile | S51 | Refresh to blocked state |
| DM sent to non-mutual (over limit) | S43 | Composer disabled, banner shown |
| Network failure on post publish | S22 | Toast error, draft preserved in-memory, "Post" re-enables |
| Network failure on comment post | S21 | Toast error, comment text preserved in field |
| Share post to conversation fails | BS05 | Toast "Failed to send. Try again." |
| Report submission fails | BS06 | Toast "Failed to submit. Please try again." |
| Block action fails | BS06 | Toast "Failed to block. Please try again." |
| No search results for posts | S20 | "No posts match your search" centered message |
| No search results for users (New DM) | S45 | "No users found" centered message |
| Notification tap but content deleted | S41 | Toast "This content is no longer available" |
| Group chat for cancelled gathering | S44 | Banner "This gathering was cancelled" + chat remains readable but composer disabled |
